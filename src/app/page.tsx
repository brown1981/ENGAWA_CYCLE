...
import { Settings, Send, Download, Copy, MessageSquare, Plus, Paperclip, X, Image as ImageIcon, Square } from "lucide-react";
  ...
  return (
    <main className="flex h-screen bg-transparent overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full relative">
        {/* Header - Improved Visibility (v13) */}
        <header className="flex justify-between items-center px-8 py-6 z-10 transition-all">
          ...
          <div className="flex gap-4 items-center">
            {messages.length > 0 && (
              <div className="text-[10px] opacity-40 font-bold uppercase tracking-[0.2em] mr-4 select-none">
          ...
          {messages.length === 0 ? (
            ...
            messages.map((m, idx) => {
              ...
              return (
                ...
                >
                  <div className={`
                    chat-bubble px-6 py-5 rounded-[2rem] shadow-sm max-w-[90%] transition-transform duration-300
                    ${m.role === 'user' 
                      ? 'bg-accent text-white rounded-tr-none shadow-accent/20 border border-transparent dark:border-white/10' 
                  ...
                  <div className="flex gap-4 px-4 opacity-0 group-hover:opacity-40 transition-opacity duration-300">
          ...
          {isLoading && !streamingContent && (
            <div className="flex items-start">
              <div className="bg-zinc-100 dark:bg-zinc-900 px-6 py-5 rounded-[2rem] rounded-tl-none animate-pulse-slow">
                <span className="text-[10px] font-bold tracking-[0.2em] uppercase opacity-40">Synapsing...</span>
              </div>
            </div>
          )}
          ...
          <div className="max-w-3xl mx-auto w-full pointer-events-auto">
            <div className="relative group">
              ...
              <div className="absolute right-5 bottom-5 flex gap-3">
                ...
                <button 
                  onClick={handleSend}
                  className={`p-3 transition-all hover:scale-105 active:scale-95 shadow-xl ${isLoading ? 'bg-red-500 shadow-red-500/20' : 'bg-accent shadow-accent/20'} text-white rounded-[1.2rem] disabled:opacity-20 disabled:grayscale`}
                >
        ...
        <SettingsModal 
          ...
          apiKey={apiKey}
          setApiKey={() => {}} // Interface compatibility, internal handles persistence
          model={model}
          setModel={setModel}
        />
      </div>
